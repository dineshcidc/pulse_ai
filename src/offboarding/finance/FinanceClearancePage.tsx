...
import {
  Wallet, Search, ChevronDown, ChevronRight, Clock3, CheckCircle2,
  PauseCircle, ShieldAlert, Lock, Inbox,
} from 'lucide-react'

/*
 * Finance Manager › Finance Clearance — Screen F1 (Queue).
 *
 * Employees needing final settlement. Finance runs IN PARALLEL with Manager and
 * IT, and handles dues recovery, final pay, reimbursements and F&F. Same gate:
 *   • awaiting-cto — CTO hasn't approved → view-only.
 *   • pending      — needs Finance clearance ("Clear").
 *   • cleared      — settlement completed.
 *   • on-hold      — blocked (e.g. dues/advance recovery pending) with a reason.
 *
 * Row → opens Screen F2 (detail + finance clearance) via onOpen.
 */
...
export type FinanceClearanceCase = {
  id: string
  name: string
  code: string
  designation: string
...
export default function FinanceClearancePage({ onOpen }: { onOpen?: (id: string) => void }) {
  const [query, setQuery]   = useState('')
  const [filter, setFilter] = useState<Filter>('all')

  const counts = useMemo(() => ({
    all:            MOCK_FIN.length,
    pending:        MOCK_FIN.filter(r => r.status === 'pending').length,
    'awaiting-cto': MOCK_FIN.filter(r => r.status === 'awaiting-cto').length,
    'on-hold':      MOCK_FIN.filter(r => r.status === 'on-hold').length,
    cleared:        MOCK_FIN.filter(r => r.status === 'cleared').length,
  }), [])
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          <h1 className="font-bold mb-1" style={{ fontSize: 22, color: C.navy, letterSpacing: '-0.3px' }}>Finance Clearance</h1>
          <p style={{ fontSize: 13.5, color: '#787878', fontWeight: 500 }}>Complete the final settlement for exiting employees</p>
        </div>
        <div className="flex items-center justify-center rounded-2xl flex-shrink-0" style={{ width: 60, height: 60, backgroundColor: 'rgba(99,102,241,0.07)', backgroundImage: 'radial-gradient(circle, rgba(99,102,241,0.22) 1px, transparent 1px)', backgroundSize: '8px 8px', border: '1px solid rgba(99,102,241,0.14)' }}>
          <div style={{ animation: 'obFloat 4s ease-in-out infinite' }}>
            <Wallet size={26} strokeWidth={1.5} style={{ color: C.indigoDeep }} />
          </div>
        </div>
      </div>
        ...
        <div style={{ overflowX: 'auto' }} className="scrollbar-hide">
          <div style={{ minWidth: TABLE_MIN }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: GRID, gap: 12, alignItems: 'center', padding: '13px 22px', borderBottom: `1px solid ${C.border}`, background: '#FAFBFE' }}>
              {['Employee', 'Designation', 'Reason', 'Last Working Day', 'Finance Clearance', 'Action'].map((h, i) => (
...
function EmptyState() {
  return (
    <div style={{ padding: '56px 24px', textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 15, fontWeight: 700, color: C.navy, marginBottom: 6 }}>No employees found</div>
