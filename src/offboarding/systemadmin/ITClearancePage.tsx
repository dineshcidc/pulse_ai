 ...
 *
 * Employees needing IT clearance. IT runs IN PARALLEL with Manager and Finance,
 * and collects assets / revokes access on or around the last working day.
 * Same gate as the other clearances:
 *   • awaiting-cto — CTO hasn't approved → view-only.
 *   • pending      — needs IT clearance ("Clear").
 *   • cleared      — assets recovered & access revoked.
 *   • on-hold      — blocked (e.g. asset not returned) with a reason.
 *
 * Row → opens Screen S2 (detail + IT clearance) via onOpen.
 */
...
export type ITClearanceCase = {
  ...
  submittedOn: string
  // ── detail-page fields (S2) ──
  manager: string
  doj: string
  email: string
  phone: string
  notes: string             // employee's detailed reason / notes
  workMode: string          // Remote / Hybrid / On-site — hints at asset return path
...
export default function ITClearancePage({ onOpen }: { onOpen?: (id: string) => void }) {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          <h1 className="font-bold mb-1" style={{ fontSize: 22, color: C.navy, letterSpacing: '-0.3px' }}>IT Clearance</h1>
          <p style={{ fontSize: 13.5, color: '#787878', fontWeight: 500 }}>Recover assets and revoke access for exiting employees</p>
        </div>
        <div className="flex items-center justify-center rounded-2xl flex-shrink-0" style={{ width: 60, height: 60, backgroundColor: 'rgba(99,102,241,0.07)', backgroundImage: 'radial-gradient(circle, rgba(99,102,241,0.22) 1px, transparent 1px)', backgroundSize: '8px 8px', border: '1px solid rgba(99,102,241,0.14)' }}>
          <div style={{ animation: 'obFloat 4s ease-in-out infinite' }}>
            <MonitorCheck size={26} strokeWidth={1.5} style={{ color: C.indigoDeep }} />
          </div>
        </div>
      </div>
        ...
        <div style={{ overflowX: 'auto' }} className="scrollbar-hide">
          <div style={{ minWidth: TABLE_MIN }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: GRID, gap: 12, alignItems: 'center', padding: '13px 22px', borderBottom: `1px solid ${C.border}`, background: '#FAFBFE' }}>
              {['Employee', 'Designation', 'Reason', 'Last Working Day', 'IT Clearance', 'Action'].map((h, i) => (
