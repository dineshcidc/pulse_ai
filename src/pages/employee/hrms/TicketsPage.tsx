...
import {
  Ticket, ClipboardList, PlusCircle, ChevronRight, ChevronDown,
  Search, Monitor, FolderOpen, User, Users, Laptop, DollarSign, LayoutGrid, X,
  Paperclip, Mail, Phone, MessageSquare, MapPin, Tag, AlertTriangle,
} from 'lucide-react'
...
const TICKETS: TicketRecord[] = [
  { id: 'TKT-2401', category: 'IT Support',      subject: 'Laptop not connecting to corporate VPN',                  createdDate: '2026-05-20', priority: 'High',     status: 'Open',        assignedTo: 'Arjun Menon', lastUpdated: '2026-05-21' },
  { id: 'TKT-2398', category: 'HR Request',      subject: 'Update emergency contact details in HRMS system',         createdDate: '2026-05-18', priority: 'Medium',   status: 'Resolved',    assignedTo: 'Priya Mehta', lastUpdated: '2026-05-20' },
  { id: 'TKT-2395', category: 'System Support',  subject: 'Unable to access HRMS portal — persistent login error',   createdDate: '2026-05-16', priority: 'Critical', status: 'In Progress', assignedTo: 'Dev Team',    lastUpdated: '2026-05-21' },
  { id: 'TKT-2389', category: 'Finance',         subject: 'Reimbursement claim for April 2026 not processed',        createdDate: '2026-05-12', priority: 'High',     status: 'Pending',     assignedTo: 'Sunita Rao',  lastUpdated: '2026-05-15' },
  { id: 'TKT-2382', category: 'Project Request', subject: 'Access request to new project repository — AMS-v3',        createdDate: '2026-05-09', priority: 'Medium',   status: 'Open',        assignedTo: 'Raj Kumar',   lastUpdated: '2026-05-09' },
  { id: 'TKT-2375', category: 'Personal',        subject: 'Work-from-home extension request for June 2026',          createdDate: '2026-05-05', priority: 'Low',      status: 'Resolved',    assignedTo: 'Priya Mehta', lastUpdated: '2026-05-07' },
  { id: 'TKT-2368', category: 'IT Support',      subject: 'Request for additional 27-inch monitor for workstation',  createdDate: '2026-04-28', priority: 'Low',      status: 'Closed',      assignedTo: 'Arjun Menon', lastUpdated: '2026-05-02' },
  { id: 'TKT-2361', category: 'HR Request',      subject: 'April 2026 salary slip not received via email',           createdDate: '2026-04-22', priority: 'Medium',   status: 'Resolved',    assignedTo: 'Sunita Rao',  lastUpdated: '2026-04-24' },
]
...
const COL    = '0.9fr 1.15fr 2.6fr 0.95fr 0.85fr 1.1fr 1.25fr 0.95fr'
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div className="grid gap-4 mb-5" style={{ gridTemplateColumns: '1fr 1fr' }}>
        {ACTIONS.map(a => {
          const isActive = activeCard === a.id
          const Icon     = a.Icon
          return (
            <button
              key={a.id}
              ...
              onClick={() => {
                setActiveCard(isActive ? null : a.id)
              ...
              style={{
                ...
                padding: '22px 24px',
                background: isActive ? a.activeBg : '#fff',
                border: `1px solid ${isActive ? a.activeBorder : C.border}`,
                borderRadius: 18,
                cursor: 'pointer',
            ...
            >
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  {isActive ? (
                    <span
                      ...
                      style={{ padding: '2px 9px', background: a.activeBorder, fontSize: 10.5, fontWeight: 700, color: a.iconColor }}
                    >
                      <span style={{ width: 5, height: 5, borderRadius: '50%', background: a.iconColor, display: 'inline-block' }} />
                      Active
                    </span>
                  ) : (
                    <span
              ...
              >
                {isActive
                  ? <ChevronDown  size={16} strokeWidth={2.2} style={{ color: a.arrowColor }} />
          ...
          <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 16, overflow: 'hidden' }}>
            ...
            <div className="flex items-center justify-between"
              ...
              <div className="flex items-center gap-1.5">
                {(['All', 'Open', 'In Progress', 'Pending', 'Resolved', 'Closed'] as const).map(s => {
            ...
            <div className="grid" style={{ gridTemplateColumns: COL, padding: '13px 20px', background: '#F7F8FC', borderBottom: `1px solid ${C.border}` }}>
              {['Ticket ID', 'Category', 'Subject', 'Created', 'Priority', 'Status', 'Assigned To', 'Updated'].map(h => (
              ...
              filtered.map((t, idx) => {
                ...
                return (
                  <div key={t.id} className="tkt-row grid items-center"
                    ...
                    <span style={{ fontSize: 13, color: '#3D4266', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingRight: 14, display: 'block' }} title={t.subject}>
                      {t.subject}
                    </span>
                    <span style={{ fontSize: 12, color: '#5A6080', fontWeight: 500 }}>{fmtDate(t.createdDate)}</span>
                    <span className="inline-flex items-center gap-1.5 rounded-full"
                      style={{ padding: '4px 10px', background: ps.bg, color: ps.color, fontSize: 11.5, fontWeight: 600, width: 'fit-content', whiteSpace: 'nowrap' }}>
                      <span style={{ width: 5, height: 5, borderRadius: '50%', background: ps.dot, display: 'inline-block', flexShrink: 0 }} />
                      {t.priority}
                    </span>
                    <span className="inline-flex items-center gap-1.5 rounded-full"
                      style={{ padding: '4px 10px', background: ss.bg, color: ss.color, fontSize: 11.5, fontWeight: 600, width: 'fit-content', whiteSpace: 'nowrap' }}>
