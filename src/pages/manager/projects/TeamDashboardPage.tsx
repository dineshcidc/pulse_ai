  ...
  return (
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 16, overflow: 'hidden', marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: COLS, padding: '9px 20px', background: C.surface, borderBottom: `1px solid ${C.border}` }}>
          {['Employee', 'Project', 'Submitted', 'Hours', 'Status', 'Actions'].map((h, i) => (
            <div key={i} style={{ fontSize: 10.5, fontWeight: 700, color: C.muted, textTransform: 'uppercase' as const, letterSpacing: 0.5 }}>{h}</div>
          ))}
        </div>
        {tsRows.map((row, i) => {
          ...
          return (
            ...
            >
              ...
              <div><span style={{ fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 6, background: sc.bg, color: sc.color }}>{sc.label}</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                <button title="View"
                  ...
                  onMouseLeave={e => { e.currentTarget.style.background = 'rgba(99,102,241,0.09)' }}
                ><Eye size={12} strokeWidth={1.8} /></button>
                <button title="Approve" onClick={() => approveTS(row.id)} disabled={row.status !== 'pending'}
                ...
                <button title="Reject" onClick={() => rejectTS(row.id)} disabled={row.status !== 'pending'}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '5fr 7fr', gap: 16, alignItems: 'start' }}>
        ...
        {/* Left 6 — Leave Requests + Recent Team Updates stacked */}
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                    <img src={row.face} alt={row.employee} style={{ width: 38, height: 38, borderRadius: 11, objectFit: 'cover', flexShrink: 0, border: `1px solid ${C.border}` }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {row.status === 'pending' && (
                        <div style={{ display: 'flex', gap: 7 }}>
                          ...
                          ><Check size={12} strokeWidth={2.5} /> Approve</button>
                          <button onClick={() => rejectLeave(row.id)}
                          ...
                          ><X size={12} strokeWidth={2.5} /> Reject</button>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 11.5, fontWeight: 700, color: C.navy, minWidth: 22, textAlign: 'right' as const }}>{l.days}d</span>
                </div>
              </div>
            )
          })}
