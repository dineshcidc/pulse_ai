...
import {
  BarChart3, ChevronDown, ClipboardList, Check, Users, ChevronLeft, ChevronRight,
} from 'lucide-react'
import { type Frequency } from './reportingData'

/**
 * Manager view of the effort/utilization report, shown under the Fill Report
 * page's "Project Efforts" tab.
 *
 * This is a trimmed copy of the Admin "Allocation Report" — manager-only:
 *   • no page title / subtext / export
 *   • no KPI cards, no Project Utilization table
 *   • filters reduced to Date Range only (no Project/Employee/Status/Billing)
 *   • keeps just the Employee Allocation Variance table
 *
...
const ALLOCS: Alloc[] = [
  ...
  { emp: 'Nina Patel',   empId: 'PLS071', project: 'Kohler Group',      billable: true,  skill: 'Angular',       experience: '4 yr',          allocation: 50,  period: '02 Jul – 09 Jul', effort: '4h/day – 20h', peak: 21, capacity: 20, expected: 40,  logged: 38  },
  { emp: 'Omar Farid',   empId: 'PLS083', project: 'Stark Industries',  billable: false, skill: 'Python',        experience: '3 yr',          allocation: 100, period: 'From 01 Jul',      effort: '8h/day',       peak: 40, capacity: 40, expected: 160, logged: 168 },
  { emp: 'Grace Kim',    empId: 'PLS090', project: 'Wayne LLC',         billable: true,  skill: 'React',         experience: '2 yr 3 mos',    allocation: 75,  period: '05 Jul – 15 Jul', effort: '6h/day – 30h', peak: 28, capacity: 30, expected: 90,  logged: 90  },
  { emp: 'Daniel Cruz',  empId: 'PLS095', project: 'Acme Co',           billable: true,  skill: 'DevOps',        experience: '5 yr',          allocation: 100, period: 'From 07 Jul',      effort: '8h/day',       peak: 44, capacity: 40, expected: 120, logged: 96  },
]
...
/* ────────────────────────── Panel ────────────────────────── */
type QuickType = 'Monthly' | 'Weekly' | 'Bi-weekly'

/* The report's frequency locks the date-range to a single cadence. */
const FREQ_TO_QUICK: Record<Frequency, QuickType> = { Weekly: 'Weekly', Biweekly: 'Bi-weekly', Monthly: 'Monthly' }

export default function EffortsReportPanel({ freq }: { freq: Frequency }) {
  const allowedQuick = FREQ_TO_QUICK[freq]
  const [quick, setQuick]       = useState<QuickType>(allowedQuick)
  ...
  function resetFilters() {
    setQuick(allowedQuick); setMonthIdx(6); setWeekIdx(0); setHalfIdx(0)
  }

  return (
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      {/* ── 4 / 8 split ── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 3fr) minmax(0, 9fr)', gap: 20, alignItems: 'start' }}>

        {/* ══ LEFT (4): Date Range filter only ══ */}
        <aside style={{ position: 'sticky', top: 0, background: '#fff', border: `1px solid ${C.border}`, borderRadius: 16, overflow: 'hidden' }}>
          ...
          <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 16 }}>
            {/* ── Date Range ── */}
            <span style={{ fontSize: 12, fontWeight: 700, color: C.navy, letterSpacing: '-0.1px' }}>Date Range</span>
            ...
            <div style={{ display: 'flex', gap: 4, padding: 4, background: C.bg, borderRadius: 10 }}>
              {(['Monthly', 'Weekly', 'Bi-weekly'] as const).map(q => {
                ...
                const enabled = q === allowedQuick
                return (
                  <button
                    key={q}
                    type="button"
                    disabled={!enabled}
                    onClick={() => enabled && setQuick(q)}
                    title={enabled ? undefined : `This is a ${allowedQuick.toLowerCase()} report`}
                    style={{
                      flex: 1, height: 34, borderRadius: 7, border: 'none',
                      cursor: enabled ? 'pointer' : 'not-allowed',
                      fontSize: 12, fontWeight: 600, transition: 'all 0.15s',
                      background: active ? '#EEF0FE' : 'transparent',
                      color: active ? C.indigoDark : enabled ? C.muted : '#C4C8D8',
        ...
        {/* ══ RIGHT (8): Employee Allocation Variance ══ */}
